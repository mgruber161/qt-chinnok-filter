...
namespace TemplateTools.ConApp
{
    partial class Program
    {
            ...
            while (input.Equals("x") == false)
            {
                ...
                if (Int32.TryParse(input, out var select))
                {
                    if (select == 1)
                    {
                        ...
                        var qtSolutions = Program.GetQTChinnokSolutions(Program.UserPath).Union(new[] { solutionPath }).ToArray();
        ...
        internal static string[] GetQTChinnokSolutions(string startPath)
        {
            var result = new List<string>();
            var qtPaths = GetQTChinnokProjects(startPath);
        ...
        internal static string[] GetQTChinnokProjects(string startPath)
        {
            return QueryDirectoryStructure(startPath, n => n.StartsWith("QT"), "bin", "obj", "node_modules");
        }
        internal static string[] GetQTChinnokPaths(string startPath, params string[] includePaths)
        {
            var qtProjects = GetQTChinnokProjects(startPath).Union(includePaths).ToArray();
        ...
        internal static string[] GetQTChinnokParentPaths(string startPath, params string[] includePaths)
        {
            var result = new List<string>();
            var qtProjects = GetQTChinnokProjects(startPath).Union(includePaths).ToArray();
            var qtPaths = qtProjects.Select(p => Program.GetParentDirectory(p))
